...
class YoloEvalOpenCV():
    ...
    def __init__(self, model_path, image_folder, annotation_file, visualize):
        """
    ...
    def run(self):
        ...
        for name, annotations in tqdm(self.annotations['images'].items()):
            ...
            image_path = self.image_folder / name
        ...
        miou_for_class = {class_name: (np.array(ious).mean(), np.array(ious).std()) for class_name, ious in
                          ious_for_class.items()}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yolo-path', required=True, type=pathlib.Path,
                        help='Expects a folder containing config.cfg, obj.names and yolo_weights.weights')
    parser.add_argument('--collection', required=True, type=pathlib.Path,
                        help='Folder of the collection, must contain folder \'images\' and file \'annotations.yaml\'')
    parser.add_argument('-v', '--visualize', action='store_true',
                        help='Shows segmentations during the evaluation for labels (blue) and predictions (red).')
    args = parser.parse_args()

    YoloEvalOpenCV(args.yolo_path, args.collection / 'images', args.collection / 'annotations.yaml',
                   args.visualize).run()
