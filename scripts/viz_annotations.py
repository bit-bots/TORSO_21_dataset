...
with open(ANNOTATION_INPUT_FILE_PICKLED, "rb") as f:
    ...
    print("Press 'a' and 'd' to move between images. 'A' and 'D' let you jump 100 images. 'q' closes. 'n' toggles not in image text")
    files = list(annos)
    files.sort()
    not_in_image = True
    i = 0
    while True:
        ...
        for a in annos[f]["annotations"]:
            if not a["in_image"]:
                ...
                if a["type"] == "robot" or a["type"] == "ball" or a["type"] == "obstacle": # bounding boxes
                    ...
                    txt_size = cv2.getTextSize(txt, cv2.FONT_HERSHEY_COMPLEX, 1, 2)
                    cv2.putText(img, txt, (int(a["vector"][0][0]-(txt_size[0][0]/2)), int(a["vector"][0][1]+(txt_size[0][1]/2))), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
